<template>
    <div>
        ...
        <div class="otherbox" :style="commentaryshow">
            <div class="talk" v-for="(item,index) in piclist" :key="item.id" name="item.id">
  ...
  <script>
    ...
    created() {
      ...
      if (res.data.data[0][0].Pro_Spe_Title1 != null) { //判断选择菜单2是否有数值，并且赋值
        this.Pro_Spe_Title1 = res.data.data[0][0].Pro_Spe_Title1 + ":"
      }
      if (res.data.data[0][0].Pro_Spe_Title2 != null) { //判断选择菜单2是否有数值，并且赋值
